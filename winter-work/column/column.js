...
async function ask_for_detail() {
    ...
    for (var i = 0; i < result2.length; i++) {
        ...
        h2.innerHTML = result2[i].article_title
        span3.innerHTML = result2[i].article_content
        span4.innerHTML ='喜欢'+ result2[i].like
        span5.innerHTML ='浏览'+ result2[i].view
        span6.innerHTML = '收藏'+result2[i].collect

        h2.id=result2[i].id
        h2.onclick=function(e){
            location.href='/web/winter-work/article/article.html'+e.target.id
        }
        div.id=result2[i].id
